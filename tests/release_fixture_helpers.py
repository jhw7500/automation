...
PRE_V172_OPENCODE_CONTEXT_BUDGET_HUNKS = {
    '.github/workflows/opencode-auto-review.yml': (
        ...
        (
            '          }\n\n          # The previous review is cut at a finding boundary, never mid-block. A half block\n          # invites the model to copy a truncated heading or evidence line, and a carryover\n          # heading that does not match a prior one fails the whole document.\n          clip_review() {\n            local text="$1"\n            if [ "${#text}" -le "$MAX_PREV_REVIEW_CHARS" ]; then\n              printf \'%s\' "$text"\n              return\n            fi\n            printf \'%s\\n\' "$text" | awk -v limit="$MAX_PREV_REVIEW_CHARS" \'\n              # No apostrophes anywhere below. This whole awk program is one single-quoted\n              # shell string, so a single apostrophe closes it and breaks the entire step.\n              # actionlint does not catch that: -shellcheck= disables shell parsing.\n              { lines[NR] = $0; cum[NR] = (NR > 1 ? cum[NR - 1] : 0) + length($0) + 1 }\n              /^#### / { heads[++hc] = NR }\n              END {\n                # The awk only runs once the caller has found the text over budget, so the\n                # trailer always fires — including for a body carrying no finding blocks,\n                # which would otherwise be cut to nothing in silence.\n                cut = 0; dropped = hc\n                for (j = 1; j <= hc; j++) {\n                  if (cum[heads[j] - 1] <= limit) { cut = heads[j] - 1; dropped = hc - (j - 1) }\n                }\n                for (i = 1; i <= cut; i++) print lines[i]\n                # The tail is Still open or Retracted (Resolved is stripped upstream), and\n                # which one is not pinned, so the count mixes open and retracted findings and\n                # must claim a status for neither. Why it is not pinned: contracts.md, #165.\n                printf "\\n- Context: previous review truncated to fit the budget; %d finding(s) omitted, do not treat them as resolved\\n", dropped\n              }\'\n          }\n',
            '          }\n',
        ),
        (
        ...
        (
            '          # `- Attestation: ` and `- Reason: ` stay bare prefixes on purpose. The authoring-time\n          # `reserved` set is anchored and only *strips* what matches, so a near-miss line the\n          # model wrote — `- Attestation: 5 trust me` — survives into the published body. The\n          # bare prefix is what keeps it out of the next round\'s context; anchoring here would\n          # let it back in wearing workflow colours.\n          PREV_REVIEW="$(printf \'%s\\n\' "$PREV_REVIEW" | grep -vE \'^(## OpenCode Review \\(latest\\)|<!-- automation:|<!-- automation-state:|- Status: |- Run: |- Reviewed: |- Validation: |- Normalization: |- Filtered candidate \\(raw\\): |- Last attempt: |- Attestation: |- Reason: |- Context: previous review truncated to fit the budget; )\' || true)"\n',
            '          PREV_REVIEW="$(printf \'%s\\n\' "$PREV_REVIEW" | grep -vE \'^(## OpenCode Review \\(latest\\)|<!-- automation:|<!-- automation-state:|- Status: |- Run: |- Reviewed: |- Validation: |- Normalization: |- Filtered candidate \\(raw\\): |- Last attempt: )\' || true)"\n',
        ),
        (
            '          # This filter deliberately omits `- Reason: `, which the previous-review one carries:\n          # the workflow writes `- Reason: <reason>` into its own failure bodies, but in a human\n          # comment it is ordinary prose, and this section is the rebuttal text the model is told\n          # to verify. Stripping it there deletes the substance of a bullet-list rebuttal.\n          HUMAN_COMMENTS="$(jq -r \\\n            \'[.[] | select((.user.type // "") != "Bot")] | .[-10:]\n             | map("@" + (.user.login // "ghost") + " (" + (.created_at // "") + "):\\n" + ((.body // "") | .[0:1500]))\n             | join("\\n\\n---\\n\\n")\' "$COMMENTS" \\\n            | grep -vE \'^(## OpenCode Review \\(latest\\)|<!-- automation:|<!-- automation-state:|- Status: |- Run: |- Reviewed: |- Validation: |- Normalization: |- Filtered candidate \\(raw\\): |- Last attempt: |- Attestation: |- Context: previous review truncated to fit the budget; )\' || true)"\n',
            '          HUMAN_COMMENTS="$(jq -r \\\n            \'[.[] | select((.user.type // "") != "Bot")] | .[-10:]\n             | map("@" + (.user.login // "ghost") + " (" + (.created_at // "") + "):\\n" + ((.body // "") | .[0:1500]))\n             | join("\\n\\n---\\n\\n")\' "$COMMENTS" \\\n            | grep -vE \'^(## OpenCode Review \\(latest\\)|<!-- automation:|<!-- automation-state:|- Status: |- Run: |- Reviewed: |- Validation: |- Normalization: |- Filtered candidate \\(raw\\): |- Last attempt: )\' || true)"\n',
        ),
        (
            '              case omit `### Still open`, `### Resolved`, and `### Retracted` entirely.\n              This rule does not apply when the previous review carries a line beginning\n              `- Context: previous review truncated to fit the budget;`. Truncation cuts at a\n              finding boundary, so it can empty a section that still holds findings: an\n              empty-looking section in a truncated review proves nothing about the active set.\n              Treat the active set as unknown rather than zero, carry over every heading you can\n              still see, and say in `### New findings` that the previous review was truncated.\n',
            '              case omit `### Still open`, `### Resolved`, and `### Retracted` entirely.\n',
        ),
    ),
}
