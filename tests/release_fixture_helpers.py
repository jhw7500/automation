...
PRE_V172_OPENCODE_CONTEXT_BUDGET_HUNKS = {
    '.github/workflows/opencode-auto-review.yml': (
        ...
        (
            '          PREV_REVIEW="$(printf \'%s\\n\' "$PREV_REVIEW" | grep -vE \'^(## OpenCode Review \\(latest\\)|<!-- automation:|<!-- automation-state:|- Status: |- Run: |- Reviewed: |- Validation: |- Normalization: |- Filtered candidate \\(raw\\): |- Last attempt: |- Attestation: |- Reason: |- Context: previous review truncated to fit the budget; [0-9]+ finding\\(s\\) omitted, do not treat them as resolved$)\' || true)"\n',
        ...
        (
            '            | grep -vE \'^(## OpenCode Review \\(latest\\)|<!-- automation:|<!-- automation-state:|- Status: |- Run: |- Reviewed: |- Validation: |- Normalization: |- Filtered candidate \\(raw\\): |- Last attempt: |- Attestation: |- Context: previous review truncated to fit the budget; [0-9]+ finding\\(s\\) omitted, do not treat them as resolved$)\' || true)"\n',
