...
**Architecture:** Preserve every v1.78.2 release-owned byte and create only documentation plus publication-guard tests on a distinct automation commit. Publish that reviewed commit through a create-only Git Data procedure, then use the existing fleet renderer and v1.78.2 verifier on one tossApp rollout. The failed automatic check remains failed; the managed fallback consumes one ordinary automatic review invocation and produces a separately verified receipt.
...
- Canary: `jhw7500/tossApp`, default branch `main`; initial observation `febb4583b608777af8e8f818fcc344aead1214cb`. Reobserve before rendering. Abort publication on base/head drift; do not retarget an approved tuple silently.
...
- [ ] Run the new tests, then `rtk python3 -m pytest -q tests/test_verify_workflow_release.py tests/test_verify_claude_rollout_fallback.py tests/test_claude_rollout_fallback.py tests/test_v1783_publication_procedure.py` and `rtk git diff --check`.
...
**Consumes:** Published v1.78.3 target; installed v1.78.2 driver; one fresh tossApp snapshot.
...
- [ ] Use a fresh private fleet workspace and the authenticated target checkout. Run the plan command in the runbook for `--repo tossApp --ref v1.78.3`; preserve its manifest. Confirm the three managed paths, prior driver pin, new target pin, deterministic branch and complete rendered diff. Stop on unmanaged drift or a conflicting existing rollout PR.
- [ ] Commit the exact rendered candidate and run the installed A/B/C review without changing the generated tree/title/body. Preserve the review binding and exact candidate tree. Present the concrete consumer branch/PR, head/base, diff, label and Codex request for approval.
- [ ] Publish through the existing fleet publisher once after approval. If the publisher recomputes a different head/base/tree, stop before remote write and rereview. Require one published/reused PR. Apply the existing `review:request` label once and request Codex once at the exact current head/base.
...
- [ ] Audit tossApp's final default-branch pins, preserve the receipt and original failed run, and record the complete chain. Only then assess existing Task completion readiness through supported Project Control; no Registry or Claim edits are permitted by this plan.
